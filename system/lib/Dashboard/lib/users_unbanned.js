...
function changeTheme(e) {
  ...
  restarts += 1;
  if (restarts >= 3) {
    window.location.href = "/users_unbanned.html";
  }
...
async function loadUsers(e, t) {
  ...
  if (a.success == 1 && ad.length === 0) {
    return Swal.fire("Users not found", "No users have been unbanned", "question").then(e => {
...
function letsUnban() {
  if (use.length === 0) {
    return Swal.fire("Users not found", "Select users before ban", "question");
  }
  Swal.fire({
    title: "Are you sure?",
    text: "You are about to ban " + use.length + " users!",
    icon: "warning",
    showCancelButton: true,
    input: "text",
    inputAttributes: {
    ...
    if (t.isConfirmed) {
      fetch("/graphql", {
        ...
        Swal.fire("Banned!", "Successfully ban " + use.length + " users.", "success").then(e => {
